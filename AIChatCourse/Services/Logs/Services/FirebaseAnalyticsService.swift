...
fileprivate extension String {
    func clean(maxCharacters: Int) -> String {
        self
            .cliped(maxCharacters: maxCharacters)
            .replaceSpacersWithUnderscores()
    }
}
...
struct FirebaseAnalyticsService: LogService{
    ...
    func trackEvent(event: any LoggableEvent) {
        ...
        for (key, value) in parameters {
            if let data = value as? Date, let string = String.convertToString(data) {
